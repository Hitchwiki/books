...
Living list of sites this repo may reuse. Add a row when we find another. Do not invent a new book until something is clearly a whole title.
...
| github.com/Couchers-org/couchers | MIT (code, not brand/blog) | describe only |
| hospitalityguide.net | CC-BY | Hospitality Exchange |
...
Drupal node bodies for Random Roads, Dumpsterdam, Moneyless, and Casa Robino are compiled from content-only dumps on `d7.bfr.ee` (no `users` / session / webform tables). Trashwiki can use `h.bfr.ee:/var/backups/mysql/sqldump/trashwiki.sql.gz` in addition to the public ZIM. Nothing under `dumps/` is committed.

## Adjacent (later or cite-only)

| Site | Notes |
| --- | --- |
| sharewiki.org | sharing initiatives; check license |
...
- CouchSurfing.com content and trademarks
- Flickr random CS photos on hospitalityguide.net
- Couchers logos, colours, long-form unlicensed essays
- Member profiles and PII on any network

## OpenCouchSurfing permissions

| Author / piece | Asked | Granted | License offered | Included |
| --- | --- | --- | --- | --- |
| *(none yet)* | | | | |
