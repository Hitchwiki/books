...
We have three wireless routers in the house but we are not using them to the full advantage. The first one is called "Tyler Durden", the second "Life Is Too Important To Be Taken" and the third one is a [Fon-hotspot](http://www.fon.com/en/info/whatsFon). The ideal would be that we use one or two of these routers to [share internet](http://sharewiki.org/en/Internet), and since the Fon-router has a great antenna attached to it, this one would be perfect to share our internet with at least 50 households. There is however one big problem with this router.
...
We actually have a great spot on the balcony for a router. There is already an electrical wire out there that we can use to power the router. So we can actually already make the power cable connect to the router and use the FON router as it is. The other router could also be used for this (we just put it on 'open', though only as long as the house and the open/shared network are separated so [sniffing](http://en.wikipedia.org/wiki/Packet_sniffer) the home-network is impossible) but the reach is far more limited and moreover the router is not as weather resistance as the FON-router, which is also designed for outdoor use.
What do you think?
See for other Fun Tasks and Things you can do that will make your Mom feel Proud also [this wiki-page](http://casarobino.org/content/fun-tasks).
