...
### Hinge \"modification\" {#hinge_modification}

#### Single rod {#single_rod}

Useful tools:

- [pliers](plier "plier"){.wikilink}
...
#### Multiple pins {#multiple_pins}

![](Sulo_dumpster.jpeg "Sulo_dumpster.jpeg"){width="300"}
...
Useful [ tools](:Category:tools " tools"){.wikilink}:
...
### Triangle lock {#triangle_lock}

Sometimes the bins are closed with a system integrated in them, needing a [triangular key](triangular_key "triangular key"){.wikilink}. The easiest way to open those is to get a triangle key or [pliers](pliers "pliers"){.wikilink} to open the bin. But if you don\'t have those, or forgot them at home, you might open the bin by sliding your arm under the lid through the side (if possible). Then reach to the lock and move the metal plate that keeps the bin shut. You might have to press a bit on the lid where the lock is so that it\'s not too hard to move.

### Brute force {#brute_force}

Useful [ tools](:Category:tools " tools"){.wikilink}:

- [bolt cutters](bolt_cutter "bolt cutter"){.wikilink}
- jimmy bar / [crowbar](crowbar "crowbar"){.wikilink}
...
It seems theoretically possible to open a lock with a [bump key](http://en.wikipedia.org/wiki/Lock_bumping). Please update if you have any tips.

See also: [How to make a bump key](http://www.metacafe.com/watch/871103/how_to_make_bump_key).

### Cages

This is general information about cages. Supermarket dumpster cages often are different from residential building trash cages.
...
### Courtyard access {#courtyard_access}
...
![Sulo_hinge_pin.png](../../images/sulo_hinge_pinpng.jpg)
